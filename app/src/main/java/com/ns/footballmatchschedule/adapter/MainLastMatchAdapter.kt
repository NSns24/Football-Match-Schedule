...
class MainLastMatchAdapter(private val matches: List<Match>, private val listener: (Match) -> Unit) : RecyclerView.Adapter<LastMatchViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LastMatchViewHolder {
        val holder = LastMatchViewHolder(LastMatchUI().createView(AnkoContext.create(parent.context, parent)))
...
class LastMatchViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    private val homeScore: TextView = view.find(text_home_score)
    private val awayScore: TextView = view.find(text_away_score)
